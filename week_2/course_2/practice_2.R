...
df <- read.table("~/Documents/GitHub/NTU-CS-X-RProject-AHsiang/week_2/course_2/test_data.csv", sep = ",", header =TRUE, stringsAsFactors = F)
getwd()
df
# 使用dim()查看df的維度(列數與欄數)
dim(df)
...
match(NA,df$Grade)
df$Grade[11] <- 990
#或用這個 -->   df$Grade[is.na(df$Grade)] <- 990
df$Grade
...
#names(df)[4] <- "ToeicGrade"   # df[]裡面要放的是位置，不可以把它當成search，把你要的"Grade"直接放進去
#如果還是想要放"Grade"，那就要寫成  names(names(df)=="Grade") <-"ToeicGrade"  ) 
#要雙引號的時候代表是針對一個名字or字串，但如果沒有要用雙引號，原則上只有在選變數才行
...
df[df$School == "A", ]
df[,df$School == "A"]       #會跑出這種結果主要是因為R他不知道你為何要用row篩選出的邏輯去篩選colmn裡面的東西，邏輯不對但語法對，所以R還是print出    
                            #df[4] == df[,4]  要記住！！！！！ 
